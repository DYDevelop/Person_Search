...
class BaseNet(nn.Module):
    def __init__(self, cfg):
        ...
        roi_heads = BaseRoIHeads(
            ...
            embedding_type=cfg.MODEL.ROI_HEAD.EMBD,
        ...
        self.embedding_type = cfg.MODEL.ROI_HEAD.EMBD,
        # loss weights
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            try:
                ...
                embeddings, _ = self.roi_heads.embedding_head(box_features['feat_res4'], box_features['feat_res5'])
                return embeddings.split(1, 0)
        else:
            # gallery
...
class BaseRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        if embedding_type == 'OIMNet':
            ...
            self.embedding_type = 'OIMNet'

        if embedding_type == 'NAE':
            self.embedding_head = NormAwareEmbeddingProj(
                    in_channels=[1024, 2048],
                    dim=256)
            self.embedding_type = 'NAE'
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        if self.embedding_type == 'OIMNet':
            class_logits, box_regression = self.box_predictor(rcnn_features['feat_res5'])
            embeddings_ = self.embedding_head(rcnn_features)
            # print(class_logits.size()) torch.Size([640, 2])
            # print(embeddings_.size()) torch.Size([640, 256])

        if self.embedding_type == 'NAE':
            _, box_regression = self.box_predictor(rcnn_features['feat_res5'])
            # print(rcnn_features.keys()) # odict_keys(['feat_res4', 'feat_res5'])
            embeddings_, class_logits = self.embedding_head(rcnn_features['feat_res4'], rcnn_features['feat_res5'])
            # print(class_logits.size()) torch.Size([640])
            # print(embeddings_.size()) torch.Size([640, 256])
        ...
        if self.training:
            ...
            if self.embedding_type == 'NAE':
                boxes, scores, embeddings, labels = \
                    self.postprocess_boxes_NAE(class_logits, box_regression, embeddings_,
                                            proposals, image_shapes, gt_det=gt_det)
